@echo off
REM Script to create S3 buckets and DynamoDB tables for Terraform backend
...
aws s3api put-bucket-encryption --bucket enterprise-terraform-state-prod --server-side-encryption-configuration "{\"Rules\":[{\"ApplyServerSideEncryptionByDefault\":{\"SSEAlgorithm\":\"AES256\"}}]}"
aws s3api put-bucket-encryption --bucket enterprise-terraform-state-stage --server-side-encryption-configuration "{\"Rules\":[{\"ApplyServerSideEncryptionByDefault\":{\"SSEAlgorithm\":\"AES256\"}}]}"
...
REM Create DynamoDB tables for state locking
echo Creating DynamoDB tables for state locking...
aws dynamodb create-table --table-name terraform-state-lock-prod --attribute-definitions AttributeName=LockID,AttributeType=S --key-schema AttributeName=LockID,KeyType=HASH --provisioned-throughput ReadCapacityUnits=1,WriteCapacityUnits=1 --region %REGION%
aws dynamodb create-table --table-name terraform-state-lock-stage --attribute-definitions AttributeName=LockID,AttributeType=S --key-schema AttributeName=LockID,KeyType=HASH --provisioned-throughput ReadCapacityUnits=1,WriteCapacityUnits=1 --region %REGION%
